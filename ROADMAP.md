...
> **Status:** 🚧 In Progress (Phase 2 — Testing & Reliability (2.1 & 2.2 done; 2.3 🟡 blocked on Chrome-150 extension-load regression — see Q7); 2.4 pending)  
> **Next Milestone:** Unblock Phase 2.3 (Q7) or proceed to Phase 2.4 (Error Injection Tests)  
...
| 2.3 | **E2E Flow Tests** | 🟡 Blocked | End-to-end test: claim campaign → inject content script → verify platform page navigation | - Uses existing WebdriverIO e2e setup  <br>- Full flow: background poll → claim → orchestrator → content script  <br>- Assertions on badge text and storage state  <br>- Runs in CI. **BLOCKED:** Chrome 150 silently refuses to load the unpacked MV3 extension in the WDIO v9.19 / chromedriver 150 session — neither `goog:chromeOptions.extensions: [base64]` nor `--load-extension` surfaces anything on `chrome://extensions/`. Probe confirmed `--load-extension` reaches the Chrome command line yet `#extensions-section` stays hidden. All existing extension-page E2E specs (`page-popup`, `page-options`, `page-new-tab`, `page-devtools-panel`, `page-side-panel`) are broken on this environment for the same reason; only `smoke.test.ts` passes (no extension needed) — see Q7. The plan (driver via popup Retry Now / Clear Errors; stub the Litoral API via WebDriver Bidi `browser.mock`; assert via popup DOM + storage) is sized and ready once loading is unblocked. |
