...
> **Status:** 🚧 In Progress (Phase 3 begun — first toehold landed: `test:unit` CI gate + stale-main workflow fixes)  
> **Next Milestone:** Phase 3.1 (GitHub Actions Workflow) — in progress; sub-task Q8 closed, stale-main migration done. Remaining 3.1 work: `.github/workflows/release.yml`; then Phase 3.2 (Chrome Web Store auto-publish), 3.3 (Firefox AMO auto-submit), 3.4 (version bumping), 3.5 (store listing sync).  
...
| # | Task | Status | Details | Acceptance Criteria |
|---|------|--------|---------|---------------------|
| 3.1 | **GitHub Actions Workflow** | 🟡 In Progress | Build on push to `main`, run tests, produce signed zips for Chrome & Firefox. Toehold landed: `.github/workflows/test-unit.yml` created (closes Q8 — `pnpm test:unit` now runs on every PR); three workflows (`lint.yml`, `build-zip.yml`, `prettier.yml`) switched from `pull_request_target` → `pull_request` to fix the stale-main checkout hazard flagged in the 2026-07-17 changelog. Still pending: `.github/workflows/release.yml`, plus the rest of Phase 3 (3.2-3.5). | - `.github/workflows/build-zip.yml` updated  <br>- `.github/workflows/release.yml` created  <br>- Builds run on every PR  <br>- Zips uploaded as artifacts |
