...
> **Status:** 🚧 In Progress (Phase 3.1 ✅ done — release.yml + test-unit.yml + stale-main workflow fixes; Phase 3.2-3.5 pending)  
> **Next Milestone:** Phase 3.2 (Chrome Web Store Auto-Publish) — needs `CHROME_CLIENT_ID` / `CHROME_CLIENT_SECRET` / `CHROME_REFRESH_TOKEN` / `CHROME_EXTENSION_ID` stored as GitHub Secrets (Q5 deployment-mode decision needs sign-off first). Then 3.3 (Firefox AMO), 3.4 (version bumping), 3.5 (store listing sync).  
