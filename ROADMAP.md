...
> **Status:** 🚧 In Progress (Phase 3.1 ✅ done — release.yml + test-unit.yml + stale-main workflow fixes; Phase 3.4 ✅ done — `bump_version.sh` wired into `release.yml` with auto-commit-back + tag + release; Phase 3.2/3.3/3.5 pending)  
> **Next Milestone:** Phase 3.2 (Chrome Web Store Auto-Publish) is next blocker — needs `CHROME_CLIENT_ID` / `CHROME_CLIENT_SECRET` / `CHROME_REFRESH_TOKEN` / `CHROME_EXTENSION_ID` stored as GitHub Secrets (Q5 deployment-mode decision still open). Then Phase 3.3 (Firefox AMO — needs `AMO_API_KEY` / `AMO_API_SECRET` secrets), Phase 3.5 (Store Listing Sync + zip-filename-determinism fix: `packages/zipper/index.mts` still derives `extension-YYYYMMDD-HHMMSS.{zip,xpi}` from `new Date()` not `package.json` version).  
